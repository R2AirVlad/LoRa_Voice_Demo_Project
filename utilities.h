...
#define LORA_RADIO_TASK_TX_BIT  0x02  // Флаг-Бит нужен для идентификатора источника прерывания RTOS при передаче

// Настройка подключения пинов микрофона I2S INMP441 к пинам контроллера. Напряжение питания 3,3 В (вывод VDD) можно взять с любого подходящего вывода платы, а вывод GND соединить с GND платы.
#define AUDIO_MIC_SD            2 // соединяем вывод SD микрофона к GPIO 2 платы
#define AUDIO_MIC_WS            15 // соединяем вывод WS микрофона к GPIO 15 платы
#define AUDIO_MIC_SCK           4 // соединяем вывод SCK микрофона к GPIO 4 платы
...
#define AUDIO_TASK_RECORD_BIT   0x02    // Бит флага для Задачи (таска) старта записи звука 
